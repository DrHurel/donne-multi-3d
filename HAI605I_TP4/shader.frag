...
void main (void) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if(dotLN >0.){
            ...
            // Solution 1 : pas facile à trouver, mais rapide à écrire:
            dotLN = floor(dotLN * float(levels)) / float(levels);
            // Solution 2, plus intuitive:
            /*for (int iLevel = 0; iLevel < levels; iLevel++) {
                // Pour chaque "niveau" possible, je verifie si je depasse un seuil
                if (dotLN < float(iLevel+1) / float(levels)) {
                    // Si oui, je bloque la valeur a ce seuil, puis je coupe la boucle
                    dotLN = float(iLevel) / float(levels);
                    break;
                }
            }*/
