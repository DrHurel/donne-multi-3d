...
Shader::Shader()
    : shaderProgram(0), vertexShader(0), fragmentShader(0), vertexShaderSize(0),
...
void Shader::loadFromFile(const string &vertexShaderFilename,
                          const string &fragmentShaderFilename) {
  const GLchar *vertexShaderSource;
  const GLchar *fragmentShaderSource;
  if (vertexShaderFilename != "")
    vertexShaderSource =
        readShaderSource(vertexShaderFilename, vertexShaderSize);
  if (fragmentShaderFilename != "")
    fragmentShaderSource =
        readShaderSource(fragmentShaderFilename, fragmentShaderSize);
...
unsigned int Shader::getShaderSize(const string &filename) {
  ...
  if (fd != -1) {
    ...
    throw ShaderException(string("getShaderSize: bad Shader File Name") +
                          filename);
#else
...
GLchar *Shader::readShaderSource(const string &shaderFilename,
                                 unsigned int &shaderSize) {
  shaderSize = getShaderSize(shaderFilename);
  FILE *fh = fopen(shaderFilename.c_str(), "r");
  if (!fh)
    throw ShaderException(string("readShaderSource : Bad Shader Filename ") +
                          shaderFilename);
  ...
  fclose(fh);
  //  if (ferror (fh))
  //  throw ShaderException (string ("readShaderSource : error while reading
  //  file") + shaderFilename);
