...
void Mesh::clearGeometry() { vertices.clear(); }

void Mesh::clearTopology() { triangles.clear(); }

void Mesh::computeTriangleNormals(std::vector<Vec3Df> &triangleNormals) {
...
void Mesh::recomputeSmoothVertexNormals(unsigned int normWeight) {
  ...
  std::vector<Vec3Df>::iterator itNormal = triangleNormals.begin();
...
void Mesh::collectOneRing(
    std::vector<std::vector<unsigned int>> &oneRing) const {
  ...
  for (unsigned int i = 0; i < triangles.size(); i++) {
    ...
    for (unsigned int j = 0; j < 3; j++) {
      ...
      for (unsigned int k = 1; k < 3; k++) {
        ...
        if (find(oneRing[vj].begin(), oneRing[vj].end(), vk) ==
            oneRing[vj].end())
