...
ostream &operator<<(ostream &output, const Vertex &v) {
  output << v.getPos() << endl << v.getNormal() << endl;
  return output;
}
...
void Vertex::scaleToUnitBox(vector<Vertex> &vertices, Vec3Df &center,
                            float &scaleToUnit) {
  computeAveragePosAndRadius(vertices, center, scaleToUnit);
  for (unsigned int i = 0; i < vertices.size(); i++)
    vertices[i].setPos(Vec3Df::segment(center, vertices[i].getPos()) /
                       scaleToUnit);
}
