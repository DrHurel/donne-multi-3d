...
public:
  ...
  void setLevelsRef(int s) { glUniform1iARB(levelsRefLocation, s); }
...
inline void glVertexVec3Df(const Vec3Df &v) { glVertex3f(v[0], v[1], v[2]); }

inline void glNormalVec3Df(const Vec3Df &n) { glNormal3f(n[0], n[1], n[2]); }
...
void setShaderValues() {
  ...
  phongShader->setLevelsRef(levelsRef);
}
...
void drawMesh(bool flat) {
  ...
  for (unsigned int i = 0; i < T.size(); i++) {
    ...
    if (flat) {
      Vec3Df normal = Vec3Df::crossProduct(
          V[t.getVertex(1)].getPos() - V[t.getVertex(0)].getPos(),
          V[t.getVertex(2)].getPos() - V[t.getVertex(0)].getPos());
      normal.normalize();
      glNormalVec3Df(normal);
    }
    for (unsigned int j = 0; j < 3; j++)
...
void init(const std::string &filename) {
  ...
  if (glewGetExtension("GL_ARB_vertex_buffer_object") != GL_TRUE) {
    std::cerr << "Driver does not support Vertex Buffer Objects" << std::endl;
    exit(EXIT_FAILURE);
  }
...
void idle() {
  ...
  if (currentTime - lastTime >= 1000.0f) {
    ...
    if (mode == Solid)
      sprintf(FPSstr, "TP2: %d tri. - solid shading - %d FPS.", numOfTriangles,
              FPS);
    else if (mode == Phong)
      sprintf(FPSstr, "TP2: %d tri. - Phong shading - %d FPS.", numOfTriangles,
              FPS);
...
void printUsage() {
  std::cerr << std::endl
            ...
            << " A/a: Increase/Decrease ambient reflection" << std::endl
            << " D/d: Increase/Decrease diffuse reflection" << std::endl
            << " S/s: Increase/Decrease specular reflection" << std::endl
            << " +/-: Increase/Decrease shininess" << std::endl
