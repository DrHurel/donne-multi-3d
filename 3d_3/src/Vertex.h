...
public:
  ...
  inline Vertex(const Vec3 &pos) : position(pos), normal(Vec3(0.0, 0.0, 1.0)) {}
  inline Vertex(const Vec3 &pos, const Vec3 &normal)
  ...
  inline bool operator==(const Vertex &v) {
    return (v.position == position && v.normal == normal);
  }
  ...
  static void scaleToUnitBox(std::vector<Vertex> &vertices, Vec3 &center,
                             float &scaleToUnitBox);
